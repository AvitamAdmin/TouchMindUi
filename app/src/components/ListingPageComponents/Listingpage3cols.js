...
const Listingpage3cols = ({
  ...
  //       acc[field.value] = ""; // Set each field to an empty string initially
  //       return acc;
  //     }, {});
  //     setSearchValues(initialValues); // Set the initial state with empty strings

  //     const message = {};
  //     fields.forEach((f) => {
  //       if (initialValues[f.value]?.trim()) {
  //         message[f.value] = initialValues[f.value]; // Empty message since all values are empty
  //       }
  //     });

  //     dispatch(getFilterInputValue(message));
  //     setShowInputs(false);
  //     // Dispatch with empty values
  //     console.log("Empty message object for API request:", message);
  //   }
  // }, [fields, searchValues, dispatch]);
